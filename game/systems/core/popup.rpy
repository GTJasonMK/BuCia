...
init python:
    ...
    def debug_popup(message, popup_type="info"):
        """
        调试弹窗 - 在控制台中使用

        用法（按 Shift+O 打开控制台）：
            debug_popup("测试消息")
            debug_popup("线索测试", "clue")
            debug_popup("角色测试", "character")
            debug_popup("地点测试", "location")
            debug_popup("警告测试", "warning")
        ...
        title = type_titles.get(popup_type, "提示")
        show_popup(title, message, popup_type)
    ...
    def debug_popup_all():
        ...
        show_popup("信任度变化", "罗琳达 +10", "trust")
        show_popup("警告", "危险区域", "warning")
        show_popup("提示", "这是一条测试信息", "info")
        renpy.notify("已添加8个弹窗到队列")
